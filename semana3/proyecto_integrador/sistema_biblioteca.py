...
class Bibiblioteca:
    ...
    def agregar_usuario(self, usuario):
        if usuario.id_usuario in self.usuarios:
            return False, f"Error: Ya existe un usuario con este ID: '{usuario.id_usuario}'"
        
        self.usuarios[usuario.id_usuario] = usuario
        return True, f"El usuario '{usuario.nombre}' se agregó correctamente."
    
    def prestar_libro(self, id_usuario, id_libro, dias_prestamo, archivo="prestamos.json"):
    ...
    def guardar_prestamos_json(self, archivo):
        ...
        try:
            with open(archivo, "w") as file:
                json.dump([prestamo.to_dict() for prestamo in self.prestamos], file, indent=4)
